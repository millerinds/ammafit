        ...
        <form id="layout-form" onSubmit={handleSubmit} className="min-h-0 flex-1 overflow-y-auto p-4 sm:p-6">
          <div className="mx-auto max-w-4xl space-y-6">
            <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
              ...
              <details className="mt-3 rounded-xl border border-slate-200 p-3"><summary className="cursor-pointer text-sm text-slate-600">Usar URL externa para a logo</summary>{formData.logo_object_key && <p className="mt-2 text-xs font-medium text-emerald-700">Logo enviada pelo painel. Preencha abaixo somente se quiser substituí-la por uma URL externa.</p>}<input type="url" value={formData.logo_object_key ? '' : formData.logo_url} onChange={(event) => setFormData({ ...formData, logo_url: event.target.value, logo_object_key: '' })} placeholder="https://exemplo.com/logo.png" className="mt-2 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 outline-none" /></details>
